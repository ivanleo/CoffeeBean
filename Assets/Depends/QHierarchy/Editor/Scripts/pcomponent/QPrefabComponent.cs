...
namespace qtools.qhierarchy.pcomponent
{
    public class QPrefabComponent: QBaseComponent
    {
        ...
        public override void draw ( GameObject gameObject, QObjectList objectList, Rect selectionRect )
        {
            ...
            var info = PrefabUtility.GetPrefabInstanceStatus( gameObject );
            
            if ( info == PrefabInstanceStatus.MissingAsset ||
                 info == PrefabInstanceStatus.Disconnected  )
            {
                QColorUtils.setColor ( inactiveColor );
                GUI.DrawTexture ( rect, prefabTexture );
                QColorUtils.clearColor();
            }
            else if ( !showPrefabConnectedIcon && info != PrefabInstanceStatus.Connected )
            {
